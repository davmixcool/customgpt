...
const CustomGPTResponse = (response) => {
	this.err = response[0];
	this.response = response[1];
}

const CustomGPT = ({host = base_url, key = null} = {}) => {
  this.host = host;
  this.apiKey = key;
};

CustomGPT.prototype.create_collection = async (name) => {
	let url = `${this.host}v1/collection/create`;
	return new CustomGPTResponse(await body_request(url,{ name: name },this.apiKey,'POST'));
}

CustomGPT.prototype.delete_collection = async (id) => {
